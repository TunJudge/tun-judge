...
export class UsersService {
  ...
  getById(id: number, relations: string[] = [], select?: (keyof User)[]): Promise<User> {
    return this.usersRepository.findOneOrThrow(
      { where: { id }, relations, select },
  ...
  findOneOrThrow(user: Partial<User>, exception: Error): Promise<User> {
    return this.usersRepository.findOneOrThrow(user, exception);
  }
